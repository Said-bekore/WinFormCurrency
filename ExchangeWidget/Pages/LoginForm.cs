...
namespace ExchangeWidget.Pages
{
    public partial class LoginForm : Form
    {
        ...
        {
            var bytes = Encoding.Default.GetBytes(PasswordTextBox.Text + WinFormCurrency.Properties.Resources.Salt);
            var hash = MD5.HashData(bytes);
            var password = Convert.ToBase64String(hash);

            if (LoginTextBox.Text == WinFormCurrency.Properties.Resources.Login &&
                password == WinFormCurrency.Properties.Resources.Password)
            {
